...
import { format, isAfter, addMonths, isSameMonth, startOfMonth } from 'date-fns'
	...
	return transactions.filter(({ categoryId, date }) => {
		...
		return thisMonth
			...
			: categoryIsIncluded &&
					isAfter(
						new Date(date),
						startOfMonth(addMonths(activeDate, -11))
					) &&
	...
	(transactions, categories, activeDate) => {
		return transactions.reduce((acc, transaction) => {
			if (isSameMonth(new Date(transaction.date), activeDate)) {
				...
				cloned.category =
					categories.find(({ id }) => id === cloned.categoryId)
						?.name || ''
				acc.push(cloned)
			}
...
export const formatTransactionsForStackedBarGraph = createSelector(
	...
	(transactions, categories, activeDate) => {
		...
		const range = Array(12)
			.fill(0)
			.map((_, index) =>
				startOfMonth(addMonths(activeDate, -(11 - index)))
			)
		...
		const transactionsByCategoryByMonth = transactions.reduce(
			(byCategory, transaction) => {
				...
				if (unplannedIds.includes(transaction.categoryId)) {
					...
					byCategory[category.name].data[monthIndex] +=
						transaction.amount
				}
